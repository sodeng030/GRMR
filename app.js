...
app.get('/api/titles', async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT title FROM titles WHERE category = ?', [category]);
    ...
    try {
        const sql = `
            SELECT 
                ...
                a3.name AS a3_name
            FROM posts m
            LEFT JOIN users c ON m.CUID = c.uid
            LEFT JOIN users a1 ON m.A1UID = a1.uid
            LEFT JOIN users a2 ON m.A2UID = a2.uid
            LEFT JOIN users a3 ON m.A3UID = a3.uid
