...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _fetchActiveStatus() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/api/appointments/active?uid=${AppConfig.currentUserUid}'),
      );

      if (response.statusCode == 200) {
        final data = json.decode(utf8.decode(response.bodyBytes));
        setState(() {
          // 서버에서 내려주는 키 값에 맞춰 매핑
          _hasActiveMeeting = data['hasActiveMeeting'] ?? false;
          _meetingTitle = data['title'] ?? '약속 정보 없음';
          _minutesLeft = data['minutesLeft'] ?? 0;
