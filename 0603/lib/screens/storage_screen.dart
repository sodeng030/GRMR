...
class _StorageScreenState extends State<StorageScreen> {
  ...
  Future<void> _fetchMyPosts() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/api/user/bookmarks'),
      ...
      if (response.statusCode == 200) {
        ...
        log('▶ 보관함 서버 원본 데이터: $decodedData');
        
        List<dynamic> targetList = [];
        
        if (decodedData is Map<String, dynamic>) {
          targetList = decodedData['data'] ?? decodedData['bookmarks'] ?? decodedData['posts'] ?? [];
        } 
        ...
        setState(() {
          myPosts = targetList.map((json) => PostItem.fromJson(json)).toList();
          isLoading = false;
        });
      } else {
  ...
  Widget _buildStorageListItem(PostItem post) {
    bool isCompleted = post.state == 'completed';
