...
class _MainScreenState extends State<MainScreen> with WidgetsBindingObserver {
  ...
  void _connectSocket() {
    ...
    _socket!.onConnect((_) {
      ...
      // 약속 있을 때만 방 입장
      if (_activeAppointmentId.isNotEmpty) {  // 👈 조건 추가
      ...
      _fetchActiveStatus(); // 👈 추가: 소켓 연결되자마자 배너 즉시 갱신
      _fetchPosts();        // 👈 추가: 홈 목록도 즉시 갱신
        ...
        setState(() {
          ...
          if (members != null && members.isNotEmpty) {
            final myMember = members.firstWhere(
              (m) => m['uid'].toString() == AppConfig.currentUserUid,
              orElse: () => null,
            );
            if (myMember != null) {
              final state = myMember['state']?.toString() ?? '';
              _isStarted = state == 'ready' || state == 'moving' ||
                          state == 'arrival' || state == 'away';
            }
          }
        });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _fetchActiveStatus(); // 👈 화면 돌아올 때마다 즉시 호출
      _fetchPosts();
    }
  }
